...
git clone [https://github.com/dokatec/fiap-hackton.git](https://github.com/dokatec/fiap-hackton.git)
cd fiap-hackton
...
Painel Dinâmico de Monitoramento

Identity API (Swagger)

http://localhost:5001/swagger

Endpoint de Registo/Login JWT

Property API (Swagger)

http://localhost:5002/swagger

Gestão de Herdades e Talhões

Ingestion API (Swagger)

http://localhost:5003/swagger

Simulação de Envio de Sensores

Alert API (Swagger)

http://localhost:5004/swagger

Histórico Físico de Alertas

Kafka UI

http://localhost:8080

Painel visual de tópicos e partições

Prometheus

http://localhost:9090

Painel de monitorização de métricas

Grafana

http://localhost:3000

Dashboard de infraestrutura de rede
...
Aceda ao painel em http://localhost.

Clique no link para criar um novo cadastro. Registe o seu produtor cooperado.

Faça o login. O sistema gerará o token JWT e dará acesso ao Dashboard principal.

Passo 2: Construir o Mapa Agrícola

Vá até à aba Propriedades.

No formulário do painel esquerdo, adicione uma nova propriedade (Ex: Herdade do Vale Verde).

No painel direito, associe um novo talhão a essa propriedade (Ex: Setor Sul, Cultura: Soja, Área: 45 hectares).

Passo 3: Simulação de Sensores (Mensageria Kafka)

Vá até à aba Sensores IoT ou utilize o bloco Estado Atual dos Campos no Dashboard principal.

Localize o talhão recém-criado.

Clique no botão de Simulação Crítica (Seca) ⚠️.
...
Agora clique no ícone Simular Saudável (gotas verdes) 💧 no mesmo talhão.
